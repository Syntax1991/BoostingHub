...
Application and business rules: booster access request/review, booster access matching, lockout conflict, run/signup state machines, run create/edit/open/signup-window/cancel/start/complete, signup eligibility, roster draft/publish, attendance snapshot/update/completeness, completed-run payout settlement, run payout deducts (kept as a separate service from settlement/payout to preserve that service's cohesion), User disciplinary strikes, canonical run-detail DTO shaping, character identity/lifecycle, Battle.net connect/import/link/refresh, dashboard composition.
