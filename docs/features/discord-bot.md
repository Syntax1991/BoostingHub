...
4. Show an ephemeral multi-select with one option per eligible Character, preselecting the User's current active offers. For BOOSTER, each label shows the Character's persisted role if it already has an active offer, else its specialization-derived default (suffixed "(default)"), for information only — nothing here restricts the actual choice. LOOTBUDDY has no role dimension.
5. Submitting the select applies the offer immediately: for BOOSTER, each newly-offered Character's role resolves to its existing offer's role, else its specialization default (`handleCharacterSelect` in `signup-flow.ts`) — the server is still the sole authority and rejects a genuinely ambiguous Character (`INVALID_CHARACTER_ROLE`) rather than the bot guessing one.
6. If any offered BOOSTER Character's class can perform more than one role, the same ephemeral response is immediately replaced with up to 5 per-Character role selects (`renderRoleAdjustment` — Discord allows at most 5 action rows per message; a longer tail points the User to the Web dialog instead of imposing a product-wide offer-count limit), each defaulted to that Character's now-current role. Picking a role there (`handleRoleSelect`) rebuilds the User's complete current offer set and resubmits it with only that one Character's role changed — `setCharacterOffers` is not additive, and no Character can end up double-offered under two roles. This step is fully stateless: every render re-fetches the authoritative offer from the server, so there is no session to expire or go stale.
7. Ephemeral confirmation, or the mapped domain error (`src/discord-bot/interactions/error-copy.ts` — concise copy for known codes, otherwise the server's own message verbatim; it never invents a new eligibility rule).
