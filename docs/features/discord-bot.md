...
**Raidboost Announce**: when the signup path creates a Run channel for the first time, the bot posts one Carl-bot-style announce **before** the signup embed: embed title `<:PhoenixStarDiscord:…> Raidboost Announce <:PhoenixStarDiscord:…>` (Guild emoji name `PhoenixStarDiscord`), description `{NM|HC|MY} {loot emoji} {Difficulty} {Loot} - *Please refer to the channel name for the time* 🕒`. When the Run's `discordRolePing` flag is on (default; set at create/edit), message content also pings Guild roles named `tank`, `healer`, and `dps`. When the flag is off, the announce embed still posts without role mentions. Existing channels are never re-announced (no backfill / no re-ping on later polls). Missing emoji or roles are skipped with a warning; the announce still posts.
