...
2. Sort the CURRENT-targeted and NEXT-targeted items chronologically by `scheduledStartAt` (tie-broken by `runId`).
3. Assign the CURRENT block consecutive positions immediately after `#current-id`'s own (untouched) position, clamped below `#next-id`'s position; assign the NEXT block consecutive positions immediately after `#next-id`'s own (untouched) position.
4. Diff against each channel's live position and batch only the ones that actually changed into one `setPositions([...])` call (discord.js's `guild.channels.setPositions`) — an already-correctly-ordered category produces zero API calls.

Steps 3–4 of the first mechanism, and the whole of the second, are each independently idempotent. `sync-loop.ts` runs channel reconciliation (name + parent) first, then position reconciliation, then any signup/roster message work — and keeps a `runId -> channelId` map from the first pass so the message paths reuse the same resolved channel instead of re-resolving (and potentially re-renaming/re-moving) it a second time in the same poll.
...
**Marker channels are manual Discord infrastructure, not BoostingHub state.** They are ordinary text channels a human creates and places inside the active category; BoostingHub never creates, renames, deletes, archives, or repositions them, and never sends/edits/deletes messages inside them — it only ever reads their live `position` to know where each section starts. Their ids are pure environment configuration (`DISCORD_RUN_CURRENT_MARKER_CHANNEL_ID`/`DISCORD_RUN_NEXT_MARKER_CHANNEL_ID`), exactly like a configured category id.
...
**Unmanaged channels**: reconciliation only ever touches channels BoostingHub owns via a persisted `RunDiscordPost.runChannelId`, plus reading (never writing) the two configured marker channels. It never enumerates the category's full contents to move things around and never repositions/moves a manually created Phoenix channel merely because it happens to live inside the active category.
...
BoostingHub Run becomes OPEN (signup-available) while its raid-ID week is CURRENT or NEXT
  → per-Run Discord channel created under DISCORD_RUN_CATEGORY_ID (position corrected on the next poll)
...
Selection/roster-published notifications, `/runs` browse command, Discord-side Raid Lead/Admin actions (roster selection and Strike management stay Web-only), Discord role synchronization, preferred/ranked Character offers, Run type/progress channel-name segments (no domain field to source them from yet), Dawn Boosting integration of any kind. **Known limitation** of CURRENT/NEXT position reconciliation: new positions for a section are assigned as small integer offsets from its marker's own position, clamped so CURRENT never numerically reaches `#next-id` — with an implausibly large number of concurrent Run channels in one section, entries could tie in position near that boundary rather than each getting a strictly distinct slot; this has not been a practical concern at any realistic Run volume and is a deliberate simplification rather than an oversight.
