...
import {
  RAID_LEAD_CUT_ALLOCATION_KEY,
  allocateGold,
  calculateSettlementPool,
} from "@/services/payout-calculation";
import { RAID_LEAD_CUT_SHARE_UNITS, defaultShareUnits } from "@/services/payout-state";
...
describe("calculateSettlementPool", () => {
  ...
  it("A SHARE two full 1000g splits evenly with no dedicated cut", () => {
    const result = calculateSettlementPool({
      totalGold: 1000,
      raidLeadCutMode: "SHARE",
      entries: twoPresent,
    });
    expect(result.raidLeadCutShareUnits).toBe(0);
    expect(result.attendanceUnits).toBe(200);
    expect(result.totalSettlementUnits).toBe(200);
    expect(result.dedicatedRaidLeadPayout).toBe(0);
    expect(result.attendanceDistributedGold).toBe(1000);
    expect(result.totalAllocatedGold).toBe(1000);
    expect(result.attendancePayouts.every((row) => row.amountGold === 500)).toBe(true);
  });

  it("B KEEP two full 1000g uses 300 settlement units and conserves gold", () => {
    const result = calculateSettlementPool({
      totalGold: 1000,
      raidLeadCutMode: "KEEP",
      entries: twoPresent,
    });
    expect(result.raidLeadCutShareUnits).toBe(RAID_LEAD_CUT_SHARE_UNITS);
    expect(result.attendanceUnits).toBe(200);
    expect(result.totalSettlementUnits).toBe(300);
    expect(result.dedicatedRaidLeadPayout).toBe(334);
    expect(result.attendancePayouts.map((row) => row.amountGold)).toEqual([333, 333]);
    expect(result.attendanceDistributedGold).toBe(666);
    expect(result.attendanceDistributedGold + result.dedicatedRaidLeadPayout).toBe(1000);
    expect(result.totalAllocatedGold).toBe(1000);
  });

  it("C KEEP RL is attendee: ordinary attendance share stays separate from dedicated cut", () => {
    const result = calculateSettlementPool({
      totalGold: 1000,
      raidLeadCutMode: "KEEP",
      entries: [
    ...
    expect(leadShare).toBe(333);
    expect(result.dedicatedRaidLeadPayout).toBe(334);
    expect(leadShare + result.dedicatedRaidLeadPayout).toBe(667);
    expect(result.attendanceDistributedGold + result.dedicatedRaidLeadPayout).toBe(1000);
    expect(result.attendancePayouts.every((row) => row.attendanceId !== RAID_LEAD_CUT_ALLOCATION_KEY)).toBe(true);
  });

  it("D KEEP RL not in attendance still calculates dedicated cut", () => {
    const result = calculateSettlementPool({
      totalGold: 1000,
      raidLeadCutMode: "KEEP",
      entries: [
  ...
  it("E SHARE adds no extra Raid Lead share units", () => {
    const result = calculateSettlementPool({
      totalGold: 1_450_000,
      raidLeadCutMode: "SHARE",
      entries: twoPresent,
    ...
    expect(result.attendanceUnits).toBe(200);
    expect(result.totalSettlementUnits).toBe(200);
    expect(result.attendanceDistributedGold).toBe(1_450_000);
    expect(result.totalAllocatedGold).toBe(1_450_000);
  });

  it("F weighted 100/100/50 + KEEP 100 allocates across 350 settlement units", () => {
    const result = calculateSettlementPool({
      totalGold: 700,
      raidLeadCutMode: "KEEP",
      entries: [
    ...
    expect(result.attendanceUnits).toBe(250);
    expect(result.raidLeadCutShareUnits).toBe(100);
    expect(result.totalSettlementUnits).toBe(350);
    expect(result.dedicatedRaidLeadPayout).toBe(200);
    const byId = Object.fromEntries(result.attendancePayouts.map((row) => [row.attendanceId, row.amountGold]));
    expect(byId["att-a"]).toBe(200);
    expect(byId["att-b"]).toBe(200);
    expect(byId["att-c"]).toBe(100);
    expect(result.attendanceDistributedGold + result.dedicatedRaidLeadPayout).toBe(700);
  });

  it("G all attendance zero + KEEP gives dedicated the full pot", () => {
    const result = calculateSettlementPool({
      totalGold: 50_000,
      raidLeadCutMode: "KEEP",
      entries: [
    ...
    expect(result.attendanceUnits).toBe(0);
    expect(result.raidLeadCutShareUnits).toBe(100);
    expect(result.totalSettlementUnits).toBe(100);
    expect(result.dedicatedRaidLeadPayout).toBe(50_000);
    expect(result.attendanceDistributedGold).toBe(0);
    expect(result.attendancePayouts.every((row) => row.amountGold === 0)).toBe(true);
    expect(result.totalAllocatedGold).toBe(50_000);
  });

  it("H all attendance zero + SHARE rejects with PAYOUT_NO_ELIGIBLE_SHARES", () => {
    try {
      calculateSettlementPool({
        totalGold: 50_000,
        raidLeadCutMode: "SHARE",
