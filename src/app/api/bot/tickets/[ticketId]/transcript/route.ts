...
/**
 * POST /api/bot/tickets/:ticketId/transcript
 *
 * Persists the HTML transcript before it is sent anywhere or the channel is
 * deleted. No-op once the archive message exists.
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ ticketId: string }> }) {
  try {
    ...
    const result = await supportTicketService.recordTranscript(ticketId, await request.json());
